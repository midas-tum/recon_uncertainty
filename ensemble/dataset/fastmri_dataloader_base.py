...
class FastmriCartesianDatasetBase(object):
    ...
    def __init__(
        ...
        self.batch_size = config['fastmri_ds_batch_size']
        self.full = True if 'test' in mode else config[f'{mode}_ds']['full']
        self.mode = mode
        self.vn = config['vn']
        self.multicoil = config['multicoil']
        ...
        for ii in tqdm(range(len(data_set))):
            ...
            if 'patch_nx' in subj:
                attrs.update({'patch_nx': subj.patch_nx})
    ...
    def __getitem__(self, idx):
        ...
        if self.multicoil:
            with h5py.File(
                    os.path.join(self.root_dir, os.path.dirname(fname) + '_espirit', os.path.basename(fname)),
                    'r',
                    # libver='latest',
                    # swmr=True,
            ) as data:
                np_smaps = data['smaps_acl30']
                if self.batch_size > np_smaps.shape[0]:
                    np_smaps = np_smaps[:]  # faster than smart indexing
                else:
                    np_smaps = np_smaps[slidx]
                np_smaps = merlinpy.utils.np_ensure_complex64(np_smaps)
        else:  # singlecoil trained -> multicoil test (OOD cases)
            if not 'singlecoil' in self.mode:
                # add dimension for smaps
                np_kspace = np_kspace[:, :, np.newaxis]

                if 'brain' in self.mode:
                    with h5py.File(
                        os.path.join(self.root_dir, os.path.dirname(fname) + '_espirit', os.path.basename(fname)),
                        'r',
                        # libver='latest',
