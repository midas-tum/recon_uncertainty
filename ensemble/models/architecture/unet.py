...
class UNet(nn.Module):
    def __init__(self, dim='2D', filters=64, kernel_size=3, pool_size=2, num_layer_per_level=2, num_level=4,
                 ...
                 #n_channels, n_classes, bilinear=False):
        """
...
if __name__ == '__main__':
    import merlinpy
    import numpy as np
    config = merlinpy.experiment.loadYaml('../../../config/unet.yml', 'unet_mag_denoiser')
    config['model']['R_config']['num_level'] = 4
    model = UNet(**config['model']['R_config']).cuda()
    print(model)
    ...
    N = 96
    C = 1
    x = torch.randn(nBatch, C, M, N).cuda()
    y = model(x)
