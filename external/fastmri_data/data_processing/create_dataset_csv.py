...
image_names = sorted([os.path.join(args.dataset, f) for f in os.listdir(os.path.join(args.data_path, args.dataset)) if f.endswith(image_type) and not 'masks' in f])
...
for fname in tqdm(image_names):
     ...
     try:
          ...
          enc_info['nPE'].append(np.shape(dset['kspace'])[-1])
          enc_info['enc_x'].append(np.shape(dset['kspace'])[-2])
          enc_info['enc_y'].append(np.shape(dset['kspace'])[-1])
          enc_info['enc_z'].append(np.shape(dset['kspace'])[0])
          enc_info['rec_x'].append(np.shape(dset['reference'])[-2])
          enc_info['rec_y'].append(np.shape(dset['reference'])[-1])
          enc_info['rec_z'].append(np.shape(dset['reference'])[0])
          enc_info['enc_x_mm'].append(0)
          enc_info['enc_y_mm'].append(0)
          enc_info['enc_z_mm'].append(0)
          enc_info['rec_x_mm'].append(0)
          enc_info['rec_y_mm'].append(0)
          enc_info['rec_z_mm'].append(0)
          acq_info['systemVendor'].append(0)
          acq_info['systemModel'].append(0)
          acq_info['systemFieldStrength_T'].append(0)
          acq_info['receiverChannels'].append(np.shape(dset['kspace'])[1])
          acq_info['institutionName'].append(0)
          seq_info['TR'].append(0)
          seq_info['TE'].append(0)
          seq_info['TI'].append(0)
          seq_info['flipAngle_deg'].append(0)
          seq_info['sequence_type'].append(0)
          seq_info['echo_spacing'].append(0)
